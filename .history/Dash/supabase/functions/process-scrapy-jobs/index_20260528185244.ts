...
4. DATA CLEANING AND STANDARDIZATION:
   - NEVER store raw, unparsed, or uncleaned data in any field. All fields must be clean, structured, and ready for display.
   - For all text fields (title, company, description, summary, requirements, responsibilities, etc.), remove any HTML tags, unnecessary whitespace, special characters, or placeholder values like "N/A", "Not specified", "unknown". If a field is truly empty or not applicable, it must be an empty string "".
   - Specifically for "deadline_text": Convert all deadlines to a consistent 'Month Day, Year' format (e.g., 'May 28, 2026') or 'YYYY-MM-DD' if an exact date is not provided but implied (e.g., 'applications close end of May' -> '2026-05-31'). If no deadline is found, use an empty string.
