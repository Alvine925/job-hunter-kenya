...
function Dashboard() {
  ...
  // Filter out raw/unformatted data inline
  const jobs = useMemo(() => {
    return rawJobs.filter((job: any) => {
      const title = (job.title ?? "").trim();
      const company = (job.company ?? "").trim();
      const desc = (job.description ?? job.role_description ?? "").trim();
      
      // Skip if missing core fields
      if (!title || !company || !desc) return false;
      
      // Skip if looks like raw data (too short, only IDs, unformatted)
      if (title.length < 3 || company.length < 2 || desc.length < 20) return false;
      
      // Skip if contains raw markup or JSON
      if (/<[^>]+>/.test(title) || /"[^"]*":\s*/.test(title)) return false;
