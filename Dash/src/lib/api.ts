...
function formatInvokeError(functionName: string, error: unknown): Error {
  ...
  if (status === 404) {
    return new Error(
      `Service unavailable (404). Please try again later.`,
    );
  }
  if (status != null) {
    return new Error(
      `Service failed (${status}). Please try again later.`,
    );
  }
  ...
  if (
    ...
    rawMsg.includes("CONNECTION_CLOSED") ||
    rawMsg.includes("ERR_CONNECTION_CLOSED")
  ) {
...
async function ensureSession() {
  ...
  // Force token refresh by calling refreshSession (only if we had an expired session)
  try {
...
export async function loginWithPassword(email: string, password: string, turnstileToken?: string) {
  const res = await invokePublicFunction<any>("login", { email, password, turnstileToken });
  ...
  if (url) {
    try {
      ...
      if (host.includes("linkedin.com")) {
        ...
        if (!company || isInvalidEmployer(company)) return true;
      }
    } catch {
      return true;
    }
...
export async function checkCvUploadLimit(): Promise<UsageLimitStatus> {
  ...
  if (error) throw error;
  return (data ?? { allowed: true, reason: "" }) as UsageLimitStatus;
}

export async function checkApplicationGenerationLimit(): Promise<UsageLimitStatus> {
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) throw new Error("Not signed in");
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data, error } = await (supabase as any).rpc("check_user_limits", {
...
export async function exportMyAccountData() {
  await ensureSession();
  return invokeFunction<Record<string, unknown>>("account", { action: "export" });
}

export async function deleteMyAccount() {
  await ensureSession();
  return invokeFunction<{ success: boolean }>("account", { action: "delete" });
}
