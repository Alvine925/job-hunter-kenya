-- Migration: Add AI rate limits (chat, interview prep, manual scrape) to check_user_limits RPC
...
BEGIN
  ...
  ELSIF p_action_type = 'pack_generation' THEN
    ...
    IF v_count >= v_limit THEN
      v_allowed := false;
      v_reason := 'Weekly pack generation limit of  reached. Try again next week.';
    END IF;
  ...
  -- Check Job Coach Chat Limits (FIX 15)
  ...
  -- Check Interview Answer Limits (FIX 15)
  ...
  -- Check Manual Scrape Limits (FIX 15)
