...
Deno.serve(async (request) => {
  ...
  try {
    const result = await scrapeSite("myjobsinkenya", limit);
    return new Response(JSON.stringify({ ok: true, site: "myjobsinkenya", ...result }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (error) {
    console.error("scrape-myjobsinkenya failed", error);
    return new Response(
      JSON.stringify({
        ok: false,
        site: "myjobsinkenya",
