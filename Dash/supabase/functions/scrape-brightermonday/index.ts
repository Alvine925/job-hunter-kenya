...
Deno.serve(async (request) => {
  ...
  try {
    const result = await scrapeSite("brightermonday", limit);
    return new Response(JSON.stringify({ ok: true, site: "brightermonday", ...result }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (error) {
    console.error("scrape-brightermonday failed", error);
    return new Response(
      JSON.stringify({
        ok: false,
        site: "brightermonday",
