...
import { resetAuthReady, waitForAuthSession } from "@/lib/auth-session";
...
async function ensureSession() {
  ...
  // Force token refresh by calling getUser()
  const { data: { user }, error: userError } = await supabase.auth.getUser();
  if (userError) {
    console.error("[auth] getUser failed to refresh token:", userError);
  } else if (user) {
    const { data: { session: freshSession } } = await supabase.auth.getSession();
    if (freshSession?.access_token) return freshSession;
  }
...
export async function loginWithPassword(email: string, password: string) {
  ...
  try {
    res = await invokeFunction<any>("login", { email, password });
  } catch (err: any) {
