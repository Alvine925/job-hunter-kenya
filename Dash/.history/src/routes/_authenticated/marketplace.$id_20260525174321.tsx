...
function MarketplaceJobDetail() {
  ...
  const genMut = useMutation({
    ...
    onSuccess: (r) => {
      ...
      qc.invalidateQueries({ queryKey: ["saved-jobs"] });
      qc.invalidateQueries({ queryKey: ["scraped_jobs"] });
      // Refresh limit check after success
      checkApplicationGenerationLimit()
        .then(setLimitCheck)
        .catch(() => {});
    },
    onError: (e: Error) => {
      ...
      // Refresh limit check after error to get updated status
      checkApplicationGenerationLimit()
        .then(setLimitCheck)
        .catch(() => {});
    },
  ...
  // Wrapper functions to check rate limit before mutations
  const handleGenerateLetter = async () => {
    // Re-check the limit before attempting
    try {
      const limit = await checkApplicationGenerationLimit();
      setLimitCheck(limit);
      if (!limit.allowed) {
      ...
      console.error("Limit check error:", e);
      // If the check itself fails, still try the mutation (server will enforce)
    }
    genMut.mutate();
  };
  ...
  return (
    ...
    <JobDetailView
      ...
      onApply={handleGenerateLetter}
      onPack={handleGeneratePack}
      ...
      onInterviewGenerate={handleGenerateInterview}
