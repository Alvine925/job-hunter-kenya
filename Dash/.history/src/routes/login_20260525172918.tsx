...
export const Route = createFileRoute("/login")({
  head: () => ({
    ...
    meta: [
      { title: "Sign In - Tellus Job Intelligence" },
      { name: "description", content: "Sign in to Tellus to manage your job matches, automate applications, and prepare for interviews." },
    ],
  }),
  component: Login,
});
...
function HeroPanel({ mode, className }: { mode: AuthMode; className?: string }) {
  return (
    <div className={cn("relative min-h-0 overflow-hidden lg:h-full", className)}>
      <AuthHeroCarousel mode={mode === "forgot" ? "signin" : mode} fillHeight className="absolute inset-0 size-full" />
    </div>
  );
}
    ...
    return (
      <div className="mx-auto w-full max-w-md">
        <div className="mb-8 flex items-center gap-2 lg:hidden">
          ...
          <span className="text-lg font-bold">Tellus</span>
        </div>
...
function Login() {
  ...
  const handleEmail = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "signup") {
        ...
        // Auth succeeded — show the loading overlay for transition
        setOverlay("signin");
        await ensureAuthSessionReady();
        clearUserScopedQueries(queryClient);
  ...
  const formProps = {
    ...
    loading,
    error,
    onFullNameChange: (v: string) => { setFullName(v); setError(null); },
    onEmailChange: (v: string) => { setEmail(v); setError(null); },
    onPasswordChange: (v: string) => { setPassword(v); setError(null); },
    onSubmit: handleEmail,
    onGoogle: handleGoogle,
    onToggleMode: () => { handleToggleMode(); setError(null); },
    onForgotPasswordClick: () => { handleForgotPasswordClick(); setError(null); },
  };
      ...
      <div className="min-h-screen bg-background flex flex-col justify-center px-6 py-8 sm:px-12">
        <div className="mx-auto w-full max-w-sm text-center space-y-5 animate-in fade-in duration-300">
          <div className="flex items-center gap-2 justify-center mb-2">
            <div className="h-3 w-3 rounded-full bg-[#F4986C]" />
